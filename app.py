...
class PlannerApp(tk.Tk):
    def __init__(self):
        ...
        self.db = Database(app_folder / DB_FILE)
    ...
    def create_style(self):
        ...
        style.configure("KPI.TLabel", font=("Segoe UI", 15, "bold"))
    ...
    def create_ui(self):
        ...
        toolbar.pack(fill="x")
        ttk.Button(toolbar, text="Import Excel", command=self.import_excel).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Validate", command=self.validate_data).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Generate Plan", command=self.generate_plan).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Clear Plan", command=self.clear_plan).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Export Plan", command=self.export_plan).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Create Slip PDF", command=self.create_slip_pdf).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Print Small Slips", command=self.print_small_slips).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Refresh", command=self.refresh_all).pack(side="left", padx=4)
        ...
        self.slip_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.dashboard_tab, text="Dashboard")
        self.notebook.add(self.plan_tab, text="Production Plan")
        self.notebook.add(self.slip_tab, text="Machine Slips")

        self.create_dashboard()
        self.create_plan_tab()
        self.create_slip_tab()

        self.status = tk.StringVar(value="Ready")
        ttk.Label(self, textvariable=self.status, relief="sunken", anchor="w").pack(fill="x", side="bottom")

    def create_dashboard(self):
        kpi_frame = ttk.Frame(self.dashboard_tab, padding=20)
        kpi_frame.pack(fill="x")
        self.kpi_vars = {
            "schedules": tk.StringVar(),
            "parts": tk.StringVar(),
            "operations": tk.StringVar(),
            "machines": tk.StringVar(),
            "plan_rows": tk.StringVar(),
            "exceptions": tk.StringVar(),
        }
        labels = [
            ("Schedules", "schedules"), ("Parts", "parts"),
            ("Operations", "operations"), ("Machines", "machines"),
            ("Plan Rows", "plan_rows"), ("Exceptions", "exceptions")
        ]
        for index, (label, key) in enumerate(labels):
            box = ttk.LabelFrame(kpi_frame, text=label, padding=16)
            box.grid(row=index // 3, column=index % 3, padx=8, pady=8, sticky="nsew")
            ttk.Label(box, textvariable=self.kpi_vars[key], style="KPI.TLabel").pack()
        for column in range(3):
            kpi_frame.columnconfigure(column, weight=1)

        info = (
            "Workflow: Import Excel → Validate → Generate Plan → Export Plan.\n"
            "Machine recommendations support 15 machines per operation in the Excel template. "
            "The database design is normalized, so more machines can be added later."
        )
        ttk.Label(self.dashboard_tab, text=info, padding=20, wraplength=900).pack(anchor="w")

    def create_plan_tab(self):
        columns = (
            "plan_id", "schedule_id", "customer", "shift", "machine",
            "operation", "part", "qty", "start", "end", "sequence",
            "type", "due", "note"
        )
        self.plan_tree = ttk.Treeview(
            self.plan_tab, columns=columns, show="headings"
        )
        headings = [
            "Plan ID", "Schedule ID", "Customer", "Shift", "Machine",
            "Operation", "Part", "Qty", "Start", "End", "Seq.",
            "Type", "Due", "Note"
        ]
        widths = [150, 100, 130, 90, 130, 170, 90, 75,
                  145, 145, 60, 90, 145, 220]
        for col, heading, width in zip(columns, headings, widths):
            self.plan_tree.heading(col, text=heading)
            self.plan_tree.column(col, width=width, anchor="center")
        yscroll = ttk.Scrollbar(self.plan_tab, orient="vertical", command=self.plan_tree.yview)
        xscroll = ttk.Scrollbar(self.plan_tab, orient="horizontal", command=self.plan_tree.xview)
        ...
        self.plan_tab.rowconfigure(0, weight=1)
        self.plan_tab.columnconfigure(0, weight=1)

    def create_slip_tab(self):
        controls = ttk.Frame(self.slip_tab, padding=8)
        controls.pack(fill="x")
        ttk.Label(controls, text="Slip date (YYYY-MM-DD):").pack(side="left")
        self.slip_date = tk.StringVar(value=date.today().isoformat())
        ttk.Entry(controls, textvariable=self.slip_date, width=14).pack(side="left", padx=5)
        ttk.Button(controls, text="Load Slips", command=self.refresh_slips).pack(side="left", padx=4)
        ttk.Button(controls, text="Create Decorated PDF", command=self.create_slip_pdf).pack(side="left", padx=4)
        ttk.Button(controls, text="Print Small Slips", command=self.print_small_slips).pack(side="left", padx=4)

        columns = (
            "machine", "shift", "customer", "operation", "part",
            "qty", "start", "end", "schedule"
        )
        self.slip_tree = ttk.Treeview(self.slip_tab, columns=columns, show="headings")
        headings = [
            "Machine", "Shift", "Customer", "Operation", "Part",
            "Qty", "Start", "End", "Schedule ID"
        ]
        widths = [150, 90, 130, 190, 90, 80, 155, 155, 110]
        for col, heading, width in zip(columns, headings, widths):
            self.slip_tree.heading(col, text=heading)
            self.slip_tree.column(col, width=width, anchor="center")
        self.slip_tree.pack(fill="both", expand=True, padx=8, pady=8)

    def import_excel(self):
        path = filedialog.askopenfilename(
            title="Select Production Planning Excel",
        ...
        try:
            self.status.set("Importing Excel...")
            self.update_idletasks()

            report = self.db.import_workbook(path)
            self.refresh_all()
            ...
            warning_lines = report["warnings"]
            warning_preview = warning_lines[:20]
            warning_count = report["warning_count"]

            message = (
                "Excel data imported successfully.\n\n"
                + "\n".join(count_lines)
                + f"\nBlank rows skipped: {report['blank_rows_skipped']}"
                + f"\nPlaceholder machine rows skipped: {report['placeholder_rows_skipped']}"
                + f"\nOperations without recommended machines: {report['operations_without_machines']}"
            ...
            if warning_preview:
                ...
                if warning_count > 20:
                    message += (
                        f"\n\n...and {warning_count - 20} more warnings."
                    )
    ...
    def validate_data(self):
        ...
        if errors:
            ...
            if len(errors) > 30:
                message += f"\n\n...and {len(errors) - 30} more issues."
            messagebox.showerror(APP_NAME, message)
        else:
            messagebox.showinfo(APP_NAME, "All input data is valid.")

    def generate_plan(self):
        try:
            self.status.set("Generating production plan...")
            self.update_idletasks()

            validation_errors = self.db.validate()
            if validation_errors:
                preview = validation_errors[:30]
                message = (
                    f"Planning cannot start because {len(validation_errors)} "
                    "input issue(s) were found.\n\n"
                    + "\n".join(f"• {item}" for item in preview)
                )
                if len(validation_errors) > 30:
                    message += (
                        f"\n\n...and {len(validation_errors) - 30} more issues."
                    )
                messagebox.showerror(APP_NAME, message)
                return

            count = self.db.generate_plan()
            self.refresh_all()
            messagebox.showinfo(
                APP_NAME,
                f"Production plan generated.\n{count} operation rows created."
            )
        except Exception as exc:
            messagebox.showerror(APP_NAME, f"Planning failed:\n{exc}")
        finally:
            self.status.set("Ready")

    def clear_plan(self):
        if not messagebox.askyesno(APP_NAME, "Clear the existing production plan?"):
            return
        count = self.db.clear_plan()
        self.refresh_all()
        messagebox.showinfo(APP_NAME, f"{count} production plan rows cleared successfully.")
    ...
    def export_plan(self):
        ...
        if not path:
            return
        try:
            self.db.export_plan(path)
            messagebox.showinfo(APP_NAME, f"Plan exported successfully:\n{path}")
        except Exception as exc:
            messagebox.showerror(APP_NAME, f"Export failed:\n{exc}")

    def create_slip_pdf(self):
        try:
            selected = parse_date(self.slip_date.get())
        except ValueError as exc:
            messagebox.showerror(APP_NAME, str(exc))
            return

        default_name = f"Machine_Slips_{selected.isoformat()}.pdf"
        path = filedialog.asksaveasfilename(
            title="Save Decorated Machine Slips",
        ...
        try:
            count = self.db.create_machine_slip_pdf(selected, path)
            self.status.set(f"{count} decorated slips created.")
            messagebox.showinfo(
                APP_NAME,
                f"{count} small decorated machine slips created successfully.\n\n{path}"
            )
            try:
                os.startfile(path)
            ...
            messagebox.showerror(APP_NAME, f"Slip PDF creation failed:\n{exc}")

    def print_small_slips(self):
        try:
            selected = parse_date(self.slip_date.get())
        except ValueError as exc:
            messagebox.showerror(APP_NAME, str(exc))
        ...
        output_folder = Path(__file__).resolve().parent / "printed_slips"
        output_folder.mkdir(exist_ok=True)
        path = output_folder / f"Machine_Slips_{selected.isoformat()}.pdf"

        try:
            count = self.db.create_machine_slip_pdf(selected, str(path))
            self.status.set(f"{count} slips prepared for printing.")
            ...
            if os.name == "nt":
                try:
                    ...
                    messagebox.showinfo(
                        APP_NAME,
                        f"{count} slips sent to the default printer.\n\n"
                        "Paper size: A6 landscape / 4 x 6 inch."
                    )
                except OSError:
                    os.startfile(str(path))
                    messagebox.showinfo(
                        APP_NAME,
                        "Direct printing was unavailable. The PDF has been opened; "
                        "select A6 landscape or 4 x 6 inch paper in the print dialog."
                    )
            else:
                webbrowser.open(path.resolve().as_uri())
                messagebox.showinfo(
                    APP_NAME,
                    "The decorated slip PDF has been opened. "
                    "Print using A6 landscape or 4 x 6 inch paper."
                )
        except Exception as exc:
            messagebox.showerror(APP_NAME, f"Slip printing failed:\n{exc}")

    def refresh_dashboard(self):
        queries = {
            "schedules": "SELECT COUNT(*) FROM customer_schedules",
            "parts": "SELECT COUNT(DISTINCT part_name) FROM process_bom",
            "operations": "SELECT COUNT(*) FROM process_bom",
            "machines": "SELECT COUNT(DISTINCT machine_name) FROM machine_recommendations",
            "plan_rows": "SELECT COUNT(*) FROM production_plan WHERE plan_id <> 'EXCEPTION'",
            "exceptions": "SELECT COUNT(*) FROM production_plan WHERE plan_id = 'EXCEPTION'",
        }
        for key, query in queries.items():
            value = self.db.conn.execute(query).fetchone()[0]
            self.kpi_vars[key].set(str(value))

    def refresh_plan(self):
        for item in self.plan_tree.get_children():
            self.plan_tree.delete(item)
        for row in self.db.plan_rows():
    ...
    def refresh_slips(self):
        ...
        rows = self.db.todays_slips(selected)
        for row in rows:
            self.slip_tree.insert("", "end", values=(
                row["machine_name"], row["shift_name"],
                row["customer_name"], row["operation_name"], row["part_name"],
                row["planned_qty"], row["start_datetime"],
                row["end_datetime"], row["schedule_id"]
            ))
        self.status.set(f"{len(rows)} machine slips for {selected.isoformat()}")

    def create_slip_pdf(self, open_after=True):
        try:
            selected = parse_date(self.slip_date.get())
        except ValueError as exc:
            messagebox.showerror(APP_NAME, str(exc))
        ...
        if not rows:
            messagebox.showinfo(
                APP_NAME,
                f"No in-house operations found for {selected.isoformat()}."
            )
            return None

        default_name = f"Machine_Slips_{selected.isoformat()}.pdf"
        output_path = filedialog.asksaveasfilename(
            title="Save ELECTRO-DIP Machine Slips PDF",
            defaultextension=".pdf",
            initialfile=default_name,
            filetypes=[("PDF document", "*.pdf")]
        )
        ...
        try:
            create_machine_slips_pdf(rows, selected, Path(output_path))
            self.status.set(
                f"{len(rows)} coloured slips created for {selected.isoformat()}"
            )
            if open_after:
                try:
    ...
    def print_slip_pdf(self):
        output_path = self.create_slip_pdf(open_after=False)
        if not output_path:
            return
        try:
            if os.name == "nt":
                ...
                messagebox.showinfo(
                    APP_NAME,
                    "Slips were sent to the default Windows printer."
                )
            else:
                import subprocess
                subprocess.run(["lp", output_path], check=True)
                messagebox.showinfo(APP_NAME, "Slips were sent to the printer.")
        except Exception as exc:
            messagebox.showerror(
                APP_NAME,
                "Direct printing failed.\n"
                "Open the generated PDF and print manually.\n\n"
                f"{exc}"
            )
